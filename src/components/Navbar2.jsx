import React, { useState } from 'react';
import { Grid3x3, User } from 'lucide-react';

const Navbar = () => {
    const [isMenuOpen, setIsMenuOpen] = useState(false);

    const handleClick = () => {
        window.location.href = "/profile";
    };

    return (
        <nav className="bg-slate-950/90 backdrop-blur-md fixed w-full z-50 top-0 border-b border-slate-800 shadow-lg shadow-emerald-500/10">
            <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-3">
                <a href="/home" className="flex items-center space-x-3 group">
                    <img
                        src="https://www.pngitem.com/pimgs/m/75-751597_astrology-icon-hd-png-download.png"
                        alt="AstroSphere logo"
                        className="h-7 w-7 rounded-xl bg-slate-900 border border-slate-700 shadow-lg shadow-emerald-500/40 group-hover:scale-105 transition-transform duration-300 object-cover"
                    />
                    <span className="text-xl md:text-2xl font-bold tracking-wide ">
                        AstroSphere
                    </span>
                </a>

                <div className="flex items-center md:order-2 space-x-2">
                    <button
                        type="button"
                        className="hidden md:flex items-center gap-1 cosmic-button cosmic-button--primary rounded-full text-sm"
                        onClick={handleClick}
                    >
                        <User className="w-4 h-4" />
                        Profile
                    </button>
                    <button
                        type="button"
                        className="inline-flex items-center p-1 w-8 h-8 justify-center text-emerald-400 rounded-lg md:hidden hover:bg-slate-900/70 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 transition-all duration-300"
                        onClick={() => setIsMenuOpen(!isMenuOpen)}
                    >
                        <span className="sr-only">Open main menu</span>
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 17 14">
                            <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M1 1h15M1 7h15M1 13h15"/>
                        </svg>
                    </button>
                </div>

                <div
                    className={`items-center justify-between ${
                        isMenuOpen ? "flex" : "hidden"
                    } w-full md:flex md:w-auto md:order-1`}
                >
                    <ul className="flex flex-col p-3 md:p-0 mt-3 text-sm font-medium rounded-lg bg-slate-950 md:space-x-6 md:flex-row md:mt-0 md:bg-transparent backdrop-blur-sm md:backdrop-blur-none border border-slate-800 md:border-0">
                        <li>
                            <a
                                href="/home"
                                className="block py-2 px-3 text-slate-300 hover:text-emerald-400 transition-colors duration-300 font-semibold"
                            >
                                Home
                            </a>
                        </li>
                        <li>
                            <a
                                href="/horoscope"
                                className="block py-2 px-3 text-slate-300 hover:text-emerald-400 transition-colors duration-300 font-semibold"
                            >
                                Horoscope
                            </a>
                        </li>
                        <li>
                            <a
                                href="/birthchart"
                                className="block py-2 px-3 text-slate-300 hover:text-emerald-400 transition-colors duration-300 font-semibold"
                            >
                                Birth Chart
                            </a>
                        </li>
                        <li>
                            <a
                                href="/explore"
                                className="flex items-center gap-2 py-2 px-3 text-slate-300 hover:text-emerald-400 transition-colors duration-300 font-semibold"
                            >
                                <Grid3x3 className="w-4 h-4" />
                                Explore
                            </a>
                        </li>
                        <li className="md:hidden border-t border-slate-800 mt-2 pt-2">
                            <a
                                href="/profile"
                                className="flex items-center gap-2 py-2 px-3 text-slate-300 hover:text-emerald-400 transition-colors duration-300 font-semibold"
                            >
                                <User className="w-4 h-4" />
                                Profile
                            </a>
                        </li>
                    </ul>
                </div>
            </div>
        </nav>
    );
};

export default Navbar;
